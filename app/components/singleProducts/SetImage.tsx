import React from 'react'
import { CartProductType, SelectedImgType } from './ProductDetails'
import Image from 'next/image';

interface SetImgProps {
    cartProduct: CartProductType,
    product: any,
    handleColorSelect: (value: SelectedImgType) => void;
}

const SetImage = ({cartProduct, product, handleColorSelect}: SetImgProps) => {
  return (
    <div className='grid grid-cols-6 h-full max-h-[500px] min-h-[300px] sm:min-h-[400px]'>
        {/* multiple img */}
      <div className='flex flex-col items-center justify-center gap-2 h-full max-h-[500px] min-h-[300px] sm:min-h-[400px]'>
        {product.images.map((image: SelectedImgType) => (
            <div key={image.color} onClick={() => handleColorSelect(image)} className={`relative w-[80%] aspect-square rounded border-teal-300
            ${cartProduct.selectedImg.color === image.color ? "border-[2px]" : "border-none"}`}>
                <Image src={image.image} alt={image.color} fill className='object-contain'/>
            </div>
        ))}
      </div>
      {/* single img */}
      <div className='relative col-span-5 items-center aspect-square'>
        <Image src={cartProduct.selectedImg.image} alt={cartProduct.name} fill className='w-full h-full object-contain max-h-[500px] min-h-[300px] sm:min-h-[400px]'/>
      </div>
    </div>
  )
}

export default SetImage
